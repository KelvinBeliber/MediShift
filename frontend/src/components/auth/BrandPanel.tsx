 ...
 *
 * 1. **The poster fills the panel, always.** The artwork is rendered with
 *    `object-cover`, so it always fills the panel edge to edge with no
 *    letterboxing or blurred matting. That means the edges of the artwork can
 *    get cropped on very tall or very wide panels — keep the logo, headline,
 *    and the clinicians inside a centered safe zone (roughly the middle
 *    70-80% of the frame) when the artwork is redesigned.
...
export function BrandPanel() {
  ...
  return (
    <div className="relative overflow-hidden bg-brand-mist">
      ...
      {/* Absolutely positioned so the poster's intrinsic ratio never drives
          the grid row's height — otherwise a 900px-tall viewport would grow to
          fit the artwork and the whole page would scroll to see it. */}
